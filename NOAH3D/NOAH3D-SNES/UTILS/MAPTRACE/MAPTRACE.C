...
#define	VIEWX		((800-BLOCKSCALE*64)/2)
#define	VIEWY		(600-((600-BLOCKSCALE*64)/2))
...
#else
		_moveto (VIEWX+coordinate*BLOCKSCALE,VIEWY-(64-min)*BLOCKSCALE);
		_lineto (VIEWX+coordinate*BLOCKSCALE,VIEWY-(64-max)*BLOCKSCALE);
#endif
	}
	else
	{
...
#else
		_moveto (VIEWX+min*BLOCKSCALE,VIEWY-(64-coordinate)*BLOCKSCALE);
		_lineto (VIEWX+max*BLOCKSCALE,VIEWY-(64-coordinate)*BLOCKSCALE);
#endif
	}
#ifdef	ID_SUCKS
	PSstroke ();
...
//
	...
	NXRectFill (&rec);
		...
		if (depth)
			...
			PSsetrgbcolor (0,1,0);
#else
//shit		_setcolor(0x0F);	// White
//shit		_moveto (VIEWX+xl*BLOCKSCALE,VIEWY-(64-yl)*BLOCKSCALE);
//shit		_lineto (VIEWX+xh*BLOCKSCALE,VIEWY-(64-yl)*BLOCKSCALE);
//shit		_lineto (VIEWX+xh*BLOCKSCALE,VIEWY-(64-yh)*BLOCKSCALE);
//shit		_lineto (VIEWX+xl*BLOCKSCALE,VIEWY-(64-yh)*BLOCKSCALE);
//shit		_lineto (VIEWX+xl*BLOCKSCALE,VIEWY-(64-yl)*BLOCKSCALE);

		if(depth)
			_setcolor(0x0C);	// Red
		else
			_setcolor(0x0A);	// Green
#endif
...
void RemovePWalls (void)
{
	...
	for (y=0;y<64;y++)
		for (x=0;x<64;x++)
		{
			...
			if (front[y][x-1] == SLOTTILE)
			{	// pushing to the west
				AddSeg (di_north, x+1, y, y+1, 64+x);
				AddSeg (di_north, x, y, y+1, 64+x);
				back[y][x-2] = back[y][x];
			}
			else if (front[y][x+1] == SLOTTILE)
			{	// pushing to the east
				AddSeg (di_south, x, y, y+1, 64+x);
				AddSeg (di_south, x+1, y, y+1, 64+x);
				back[y][x+2] = back[y][x];
			}
			else if (front[y-1][x] == SLOTTILE)
			{	// pushing to the north
				AddSeg (di_east, y, x, x+1, y);
				AddSeg (di_east, y+1, x, x+1, y);
				back[y-2][x] = back[y][x];
			}
			else if (front[y+1][x] == SLOTTILE)
			{	// pushing to the south
				AddSeg (di_west, y, x, x+1, y);
				AddSeg (di_west, y+1, x, x+1, y);
				back[y+2][x] = back[y][x];
			}
			else
				Error ("pushwall at %i,%i doesn't have an adjacent slot tile",x,y);
...
//
	...
	if (draw)
	{
		...
		PSsetlinewidth (2);
#else
//shit//fuck		_setcolor(0x00);	// Black
	_setcolor(0x0F);	//White
#endif
  	}
	...
	// find the area the seg bounds
		...
		if (saveseg_p->area >= 63)
		{
			...
			&& saveseg_p->area <= 0x80+LASTDOORTILE)
			{
				...
				saveseg_p->area = 62;	// pushwall flush side
				...
				*dat++ = tile;
				...
				if (tile >=108) {
					...
					if (draw) {
						...
						color = _getcolor();
						_setcolor(0x0E);
						_ellipse (_GBORDER,VIEWX+x*BLOCKSCALE+3,(600-VIEWY)+y*BLOCKSCALE+3,
										VIEWX+(x+1)*BLOCKSCALE-3,(600-VIEWY)+(y+1)*BLOCKSCALE-3);
						_setcolor(color);
					}
					...
					if (draw) {
						...
						color = _getcolor();
						_setcolor(0x01);
						_ellipse (_GBORDER,VIEWX+x*BLOCKSCALE+2,(600-VIEWY)+y*BLOCKSCALE+2,
										VIEWX+(x+1)*BLOCKSCALE-2,(600-VIEWY)+(y+1)*BLOCKSCALE-2);
						_setcolor(color);
					}
				}
			}
...
void main (int argc, char **argv)
{
	...
	for (i=start ; i<=stop ; i++)
	{
		...
		if(draw) {
			...
			sprintf(maptitle,"Map %d: %s",i,header.name);
			_setcolor(0x0F);	// White
			_grtext(0,0,maptitle);
		} else {
			printf ("\nmap: %i (%s)\n",i,header.name);
		}
